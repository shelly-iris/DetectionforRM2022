...
bool Detector::parse_yolov5(const Blob::Ptr &blob, int net_grid, float cof_threshold,
                            vector<Rect> &o_rect, vector<float> &o_rect_cof, vector<int> &id)
{
    ...
    for (int n = 0; n < anchor_n; ++n)
        for (int i = 0; i < net_grid; ++i)
            for (int j = 0; j < net_grid; ++j)
            {
                ...
                x = (sigmoid(x) * 2 - 0.5 + j) * 480.0f / net_grid;
                y = (sigmoid(y) * 2 - 0.5 + i) * 480.0f / net_grid;
...
bool Detector::process_frame(Mat &inframe, vector<Object> &detected_objects)
{
    ...
    resize(inframe, inframe, Size(480, 480));
    cvtColor(inframe, inframe, COLOR_BGR2RGB);
    size_t img_size = 480 * 480;
    ...
    for (size_t row = 0; row < 480; row++)
    {
        for (size_t col = 0; col < 480; col++)
        {
            ...
            {
                blob_data[img_size * ch + row * 480 + col] = float(inframe.at<Vec3b>(row, col)[ch]) / 255.0f;
            }
        }
    }
    //执行预测
    ...
    for (auto &output : _outputinfo)
    {
        ...
        {
            // assert(0);
            cout << "what" << endl; //这个地方直接///识别也没啥问题break贼离谱，这个i我就不知道为啥可以等于3
            break;
        ...
        parse_yolov5(blob, s[i], _cof_threshold, origin_rect, origin_rect_cof, id);
        ++i;
    }
    //后处理获得最终检测结果
    vector<int> final_id;
    ...
    for (int i = 0; i < final_id.size(); ++i)
    {
        ...
        detected_objects.push_back(Object{
            ...
            std::to_string(id[final_id[i]]), resize_rect});
    }
    return true;
}
